...
export default function ActivityLogPage() {
  ...
  const handleLoadMore = () => {
    if (visibleCount >= activities.length) {
      alert("All logged activities are currently displayed.");
    } else {
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700/50 overflow-hidden">
        ...
        <div className="p-4 border-t border-gray-100 dark:border-slate-700/50 text-center bg-gray-50/50 dark:bg-slate-800/30">
          <button 
            ...
            className="text-sm font-semibold text-orange-600 hover:text-orange-700 transition-colors uppercase tracking-widest cursor-pointer"
          >
            {visibleCount >= filteredActivities.length ? 'All Activities Loaded ✓' : `Load Older Activity (${filteredActivities.length - visibleCount} more) ↓`}
          </button>
        </div>
      </div>
    </div>
