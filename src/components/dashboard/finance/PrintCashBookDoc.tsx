    ...
    <div className="fixed inset-0 z-[9999] bg-slate-900/80 backdrop-blur-sm overflow-y-auto p-2 sm:p-6 flex flex-col items-center">
      ...
      <div className="no-print w-full max-w-[210mm] mb-4 bg-slate-900 border border-slate-700 text-white p-3 sm:p-4 rounded-2xl shadow-xl flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h4 className="text-sm font-bold text-white">Cash Book Print Document Preview</h4>
            <p className="text-[11px] text-slate-400">A4 Optimized • Ready for printer or PDF save</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={handlePrint}
            className="flex items-center gap-1.5 px-4 py-2 bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-400 hover:to-orange-400 text-slate-950 rounded-xl text-xs font-black shadow-md cursor-pointer transition-transform active:scale-95"
          >
            <Printer size={14} />
            <span>Print Now</span>
          </button>
      ...
      {/* Printable Paper Document Container (A4 Pure Light Paper Layout) */}
      <div 
